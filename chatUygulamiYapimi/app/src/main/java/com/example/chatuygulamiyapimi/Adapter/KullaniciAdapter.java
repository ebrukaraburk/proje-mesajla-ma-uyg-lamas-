        ...
        linearGonder.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if(!TextUtils.isEmpty(txtMesaj)){
                    ...
                    mesajIstegi=new MesajIstegi(kanalId,mUID,mProfilUrl,mIsim);
                    mFireStore.collection("Mesajİstekleri").document(kullanici.getKullaniciId()).collection("İstekler").document(mUID).set(mesajIstegi).addOnCompleteListener(new OnCompleteListener<Void>() {
                        ...
                        public void onComplete(@NonNull Task<Void> task) {
                            if(task.isSuccessful()){
                                ...
                                mFireStore.collection("ChatKanallari").document(kanalId).collection("Mesajlar").document(mesajDocId).set(mData)
                                        .addOnCompleteListener(new OnCompleteListener<Void>() {
                                            @Override
                                            public void onComplete(@NonNull Task<Void> task) {
                                             if(task.isSuccessful()){
                                                 Toast.makeText(mContext, "mesaj isteginiz basari ile iletildi", Toast.LENGTH_SHORT).show();
